...
export var mathpix = {
  ...
  localImgLoad: function () {
    ...
    read.onload = function (e) {
      ...
      img.src = bs64;
      if (mathpix.enableCompression) {
        mathpix.compress(bs64, 0.5, function (compressed) {
          document.getElementById("img_mathpix").src = compressed;
          mathpix.sendToServer(compressed);
        });
      } else {
  ...
  localImgLoad_drag: function (ev) {
    ...
    if (!oFile || !(oFile instanceof Blob)) {
      msgbox.Etoast({
        type: "warning",
        message: "请拖拽本地图片文件上传",
      });
  ...
  sendToServer: function (bs) {
    ...
    mathpix.progressRun();
    texoOCR.recognize(bs).then(
      function (latex) {
        if (latex && latex.trim()) {
          $(mathpix.obj_element.el_input).val($(mathpix.obj_element.el_input).val() + latex.trim());
          mathpix.fn_refresh();
        } else {
          mathpix.obj_eject.Etoast({
            type: "warning",
    ...
    _img.onload = function () {
      ...
      _canvas.toBlob(function (blob) {
        if (blob.size > 750 * 1334) {
          //如果还大，继续压缩
          mathpix.compress(base64, rate, callback);
        } else {
          callback(base64);
        }
      }, "image/jpeg");
